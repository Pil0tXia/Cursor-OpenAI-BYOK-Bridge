...
A protocol compatibility bridge for Cursor's OpenAI-compatible BYOK path. It fixes the request/response mismatch between `/v1/chat/completions` and the Responses API, making GPT-5-class models, Azure/OpenAI-compatible gateways, and Responses-only routes usable from Cursor.

## What it is

Cursor's OpenAI-compatible BYOK path sends requests to `/v1/chat/completions`, but the request body is often closer to the Responses API format.
This Cursor behavior is tracked in the forum thread:
...
3. If the body looks like a Responses API payload, it forwards to `UPSTREAM_RESPONSES_API_URL`
...
- The Base URL configured in Cursor must be reachable by Cursor's backend. A public server or an ngrok URL works.
