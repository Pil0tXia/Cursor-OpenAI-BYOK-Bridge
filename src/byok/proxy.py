...
def apply_reasoning_config(payload: dict) -> dict:
    effort = config.REASONING_EFFORT
    if not effort:
        return payload

    model = payload.get("model")
    if config.REASONING_MODELS and model not in config.REASONING_MODELS:
        return payload

    if "reasoning" in payload and not config.REASONING_OVERRIDE:
        return payload
...
async def _handle_proxy(app: FastAPI, full_path: str, request: Request):
    ...
    if request.method.upper() == "POST" and is_chat_completions_path(incoming_path):
        ...
        if looks_like_responses_payload(payload):
            ...
            payload = apply_reasoning_config(payload)
